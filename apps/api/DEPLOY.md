...
- **Alternative:** Deploy using Docker — a `apps/api/Dockerfile` is included which uses Python 3.11 to ensure consistent, reproducible builds.

1. Go to your service > Environment > Environment Variables
2. Add `SUPABASE_URL` and `SUPABASE_KEY` (and any other keys needed)
3. Redeploy the service

If you prefer local development, copy `apps/api/.env.example` to `.env` and fill values.
