...
async def get_news(
    ...
    try:
        ...
        found = count_res.count if hasattr(count_res, 'count') and count_res.count is not None else 0
        ...
        if news_data:
            ...
            if unique_symbols:
                stocks_res = supabase.table('stocks').select('yfin_symbol, stock_name, sector, exchange, country').in_('yfin_symbol', unique_symbols).execute()
                if stocks_res and hasattr(stocks_res, 'data'):
                    stock_cache = {s['yfin_symbol']: s for s in stocks_res.data}
