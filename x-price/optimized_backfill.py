#!/usr/bin/env python3
"""
Optimized backfill script for comprehensive historical data
...
def get_date_gaps_for_stock(stock_id: str, start_date: date, end_date: date) -> List[Tuple[date, date]]:
    """
    Find date gaps for a specific stock by comparing with trading days
    Returns list of (start_gap, end_gap) tuples
    """
    try:
        # Get all existing dates for this stock
        ...
        log_message(f"Error finding gaps for stock {stock_id}: {e}", "ERROR")
        # Return full range as gap if we can't determine existing data
        return [(start_date, end_date)]

def batch_insert_prices(price_records: List[dict]) -> bool:
...
def fetch_and_fill_gaps(stock_id: str, yfin_symbol: str, stock_name: str, 
                       gaps: List[Tuple[date, date]]) -> Tuple[int, int]:
    """
    Fetch data for specific date gaps and fill them
    ...
    try:
        ...
        for gap_start, gap_end in gaps:
            ...
            try:
                ...
                df = stock.history(start=gap_start, end=gap_end + timedelta(days=1))
                ...
                # Insert batch
                if batch_records:
                    if batch_insert_prices(batch_records):
        ...
        log_message(f"Error in fetch_and_fill_gaps for {yfin_symbol}: {e}", "ERROR")
        return 0, 1

def process_stock_optimized(stock_id: str, yfin_symbol: str, stock_name: str) -> bool:
    """Process a single stock with optimized gap detection and filling"""
    try:
        log_message(f"\n{'='*60}")
        log_message(f"Processing: {stock_name} ({yfin_symbol})")
        
        # Determine date range
        # Get stock's first available date
        stock = yf.Ticker(yfin_symbol)
        
        # Try to get the earliest possible data
        early_test = stock.history(start=date(2000, 1, 1), end=date(2000, 12, 31))
        if not early_test.empty:
            # Stock has very early data, get full history
            full_history = stock.history(period="max")
            ...
            # Stock might be newer, try from 2020
            recent_history = stock.history(start=date(2020, 1, 1))
        ...
        gaps = get_date_gaps_for_stock(stock_id, start_date, end_date)
        ...
        # Fill gaps
        success_count, fail_count = fetch_and_fill_gaps(stock_id, yfin_symbol, stock_name, gaps)
...
def main():
    """Main function for optimized backfill"""
    log_message("Starting OPTIMIZED comprehensive backfill")
    ...
    if '--help' in sys.argv or '-h' in sys.argv:
        print("Usage: python optimized_backfill.py [options]")
        print("Options:")
        print("  --dry-run        Show gaps without filling them")
        print("  --stock-limit N  Process only first N stocks")
        print("  --help, -h       Show this help")
        return
    
    if '--stock-limit' in sys.argv:
        try:
            limit_index = sys.argv.index('--stock-limit')
            stock_limit = int(sys.argv[limit_index + 1])
            log_message(f"Limited to {stock_limit} stocks")
        except (IndexError, ValueError):
            log_message("Invalid --stock-limit value", "ERROR")
            return
    ...
    if not stocks:
        log_message("No active stocks found", "ERROR")
        return
    ...
    for i, stock in enumerate(stocks, 1):
        if not stock.get('yfin_symbol'):
            log_message(f"Skipping {stock.get('stock_name', 'unknown')} - no symbol", "WARNING")
            failed += 1
            continue
        
        log_message(f"\n[{i}/{len(stocks)}] Starting {stock['stock_name']}")
        ...
        if dry_run:
            ...
            try:
                ...
                if not history.empty:
                    ...
                    gaps = get_date_gaps_for_stock(stock['id'], start_date, end_date)
                    total_gap_days = sum((ge - gs).days + 1 for gs, ge in gaps)
                    log_message(f"Would fill {len(gaps)} gaps ({total_gap_days} days)")
                    successful += 1
                else:
                    log_message(f"No data available for {stock['yfin_symbol']}")
                    failed += 1
            except Exception as e:
                log_message(f"Error in dry run for {stock['yfin_symbol']}: {e}", "ERROR")
                failed += 1
        else:
            # Actually process
            if process_stock_optimized(stock['id'], stock['yfin_symbol'], stock['stock_name']):
                successful += 1
            else:
                failed += 1
        ...
        if i % 5 == 0:
            ...
            log_message(f"Progress: {i}/{len(stocks)} stocks ({rate:.1f} stocks/min)")
